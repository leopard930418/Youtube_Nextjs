...
function SearchInput({ value, onChange, resultCount, hasQuery, isLoaded }) {
  return (
    <div className="px-6 pb-6 text-center">
      <h1 className={`pb-4 text-3xl md:text-[2.75rem] font-extrabold tracking-[-0.035em] transition-all duration-[700ms] ease-[cubic-bezier(0.25,0.46,0.45,0.94)] ${isLoaded ? 'translate-y-0 opacity-100' : '-translate-y-12 opacity-0'}`}>Youtube Advice</h1>
  ...
  return (
    <>
      ...
      {filtered.length === 0 ? (
        ...
        <div className={`mx-auto grid max-w-6xl grid-cols-1 gap-5 px-6 pb-16 sm:px-28 sm:grid-cols-2 lg:grid-cols-3 transition-all duration-[700ms] ease-[cubic-bezier(0.25,0.46,0.45,0.94)] ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
          {filtered.map((topic) => (
            <TopicCard
              key={topic.id}
              topic={topic}
